...
namespace EmojiSpace
{
  ...
  {
    LOG_INFO ("EmojiTools v." + std::string (EMOJITOOLS_VERSION)
              + " constructed.");
    ...
    std::string emojiTestFileDefinition
      = this->m_assetsPath + "/emoji-test.txt";

    std::ifstream is (emojiTestFileDefinition);
    if (!is)
    {
      LOG_E << "Emoji asset test file not found. Loading hardcoded definition."
            << std::endl;
    ...
    {
      LOG_DEBUG ("Emoji asset test file found.");
      constructEmojiPropertiesMap (m_emojiPropertiesMap, is);
    }
  }

  EmojiTools::~EmojiTools () { LOG_DEBUG ("EmojiTools deconstructed."); }
  ...
  char8_t *EmojiTools::encodeUtf8Sequence (const char32_t *emojiCodePoints,
                                           size_t length, char8_t *buffer8_t)
  {
  ...
  void EmojiTools::constructEmojiPropertiesMap (
    std::map<int, EmojiPropertiesStructure> &epm, std::istream &file)
  {
    ...
    while (std::getline (file, line))
    {
               ...
               && (line.find ("# group:") == std::string::npos))
      {
        ...
        if (std::regex_search (emojiTailDescription, unicodeMatch,
                               unicodeRegex))
        {
  ...
  std::string &
  EmojiTools::getEmojiStringCharByCodePoint (char32_t *emojiCodePoints,
                                             size_t length)
  {
    if (m_isPopulated)
    {
      ...
      char8_t *end
        = encodeUtf8Sequence (emojiCodePoints, length, m_emojiBuffer);
      *end = '\0'; // Null-terminating the string
      return this->m_emojiCharacter
             = std::string (reinterpret_cast<char *> (m_emojiBuffer));
    }
    else
      // return "";
      return this->m_emojiCharacter = "";
  ...
  char8_t &
  EmojiTools::getEmojiChar8_tCharByCodePoint (char32_t *emojiCodePoints,
                                              size_t length)
  {
    std::fill (m_emojiBuffer, m_emojiBuffer + sizeof (m_emojiBuffer), 0);
  ...
  std::string &EmojiTools::getRandomEmojiFromGroup (std::string emojiGroup)
  {
    ...
    {
      int count = 0;
      std::uniform_int_distribution<> dis (1, getSizeOfGroupItems (emojiGroup)
                                                - 1);
  ...
  std::string &
  EmojiTools::getRandomEmojiFromSubGroup (std::string emojiSubGroup)
  {
    if (m_isPopulated)
    {
      int count = 0;
  ...
  std::string EmojiTools::getEmojiesFromGroup (std::string emojiGroup)
  {
    if (m_isPopulated)
    {
      ...
      for (auto &epm : m_emojiPropertiesMap)
      {
        if (epm.second.m_emojiGroup == emojiGroup)
        {
          emojies += getEmojiStringCharByCodePoint (
            epm.second.m_emojiCodePoints.data (),
            epm.second.m_emojiCodePoints.size ());
        }
      }
  ...
  std::string
  EmojiTools::getEmojiStringByIndexFromGroup (std::string emojiGroup,
                                              int index)
  {
    if (m_isPopulated)
    {
      ...
      for (auto &epm : m_emojiPropertiesMap)
      {
        if (epm.second.m_emojiGroup == emojiGroup)
        {
          if (count == index)
          {
            return getEmojiStringCharByCodePoint (
              epm.second.m_emojiCodePoints.data (),
              epm.second.m_emojiCodePoints.size ());
          }
          count++;
  ...
  std::string
  EmojiTools::getEmojiStringByIndexFromSubGroup (std::string emojiSubGroup,
                                                 int index)
  {
    if (m_isPopulated)
    {
  ...
  std::string
  EmojiTools::getEmojiSubGroupDescription (std::string emojiSubGroup)
  {
    std::stringstream ss;
